...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    let mut value_none: usize = 0;
    let mut value_some: usize = 0;
    let mut value_complete: usize = 0;
    for v in map.values() {
        if *v == Progress::None {
            value_none += 1;
        } else if *v == Progress::Some {
            value_some += 1;
        } else if *v == Progress::Complete {
            value_complete += 1;
        }
    }

    match value {
        Progress::None => value_none,
        Progress::Some => value_some,
        Progress::Complete => value_complete,
    }
}
